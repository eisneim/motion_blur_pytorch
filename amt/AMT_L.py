...
from amt.blocks.raft import (
    coords_grid,
    BasicUpdateBlock, BidirCorrBlock
)
from amt.blocks.feat_enc import (
    BasicEncoder
)
from amt.blocks.ifrnet import (
    resize,
    Encoder,
    InitDecoder,
    IntermediateDecoder
)
from amt.blocks.multi_flow import (
    multi_flow_combine,
    MultiFlowDecoder
)
...
class Model(nn.Module):
    ...
    def forward(self, img0, img1, embt, scale_factor=1.0, eval=False, **kwargs):
        ...
        corr_4, flow_4 = self._corr_scale_lookup(corr_fn, coord, 
                                                 ...
                                                 embt, downsample=1)
        # print("ft_3_", ft_3_.dtype, "flow_4", flow_4.dtype, "corr_4", corr_4.dtype)
        # residue update with lookup corr
        delta_ft_3_, delta_flow_4 = self.update4(ft_3_, flow_4, corr_4.to(dtype=img0.dtype))
        ...
        # residue update with lookup corr
        delta_ft_2_, delta_flow_3 = self.update3(ft_2_, flow_3, corr_3.to(dtype=img0.dtype))
        ...
        # residue update with lookup corr
        delta_ft_1_, delta_flow_2 = self.update2(ft_1_, flow_2, corr_2.to(dtype=img0.dtype))
